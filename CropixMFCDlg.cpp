...
BOOL CCropixMFCDlg::OnInitDialog()
{
	...
	lstReport.InsertColumn(1, _T("Связь между лицами"), LVCFMT_LEFT, 250);
	lstReport.InsertColumn(2, _T("Основание"), LVCFMT_LEFT, 150);
...
LRESULT CCropixMFCDlg::OnUpdateReport(WPARAM wParam, LPARAM lParam)
{
	for (int i = 0; i < reportItems.size(); ++i) {
		...
		int nIndex = lstReport.InsertItem(i, encoding::cp1251_to_utf16(item.сompanyName).c_str());
